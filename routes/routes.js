...
router.get('/coins/markets', async (req, res) => {
    try {
        ...
        if (!coinsData || coinsData.length === 0) {
            return handleErrorResponse(res, 404, 'No data found in MongoDB');
        }

        return res.status(200).json(coinsData);

    } catch (error) {
        handleErrorResponse(res, 500, 'Error retrieving market data from MongoDB', error);
    }
});

// Route to get coins by id(s) from /coins/markets/query?ids=bitcoin,ethereum,solana 
// GET http://localhost:3333/api/coins/markets/?ids=bitcoin,ethereum,solana
...
router.get('/coins/markets/query', async (req, res) => {
    ...
    try {
        // Redis key
        const cachedKey = COIN_MARKET_KEY;
        // Fetch data from Redis or MongoDB by id
        const coinsData = await getCryptoDataById(cachedKey, idsArr);

        if (!coinsData || coinsData.length === 0) {
            return handleErrorResponse(res, 404, `No data found for the provided ids: ${ids}`);
        }

        return res.status(200).json(coinsData);
        
    } catch (error) {
        handleErrorResponse(res, 500, 'Error retrieving data by ids from Redis or MongoDB', error);
    }
});
